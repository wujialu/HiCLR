...
class RXNConditionModel(nn.Module):
    def __init__(
        ...
        h_dim=1000,
        dropout_rate=0.5,
        c1_dim=803,  #! one-hot embedding for reaction conditions
        s1_dim=232,
        s2_dim=228,
        r1_dim=2240,
        r2_dim=1979,
        condition_dim=100,
        rxn_encoder=None, #! transformer enc-dec
        # is_train=False,
    ):
        ...
        self.not_softmax_out = self.training  
        ...
        if not self.batchnorm:
            self.lin_fp_transform = nn.Sequential(
                nn.Linear(self.fp_dim*2, 4096),
                self.activation,
            ...
            self.lin_fp_transform = nn.Sequential(
                ...
                self.activation,
                nn.Dropout(self.dropout_rate)  #! bug: fix as 0.5
            )

        self.lin_c1_dense = nn.Linear(c1_dim, self.condition_dim)
        self.lin_s1_dense = nn.Linear(s1_dim, self.condition_dim)
        self.lin_s2_dense = nn.Linear(s2_dim, self.condition_dim)
        self.lin_r1_dense = nn.Linear(r1_dim, self.condition_dim)
        self.lin_r2_dense = nn.Linear(r2_dim, self.condition_dim)

        self.lin_c1_h1 = nn.Linear(self.h_dim, 300)
        self.lin_s1_h1 = nn.Linear(self.h_dim+self.condition_dim, 300)
        self.lin_s2_h1 = nn.Linear(self.h_dim+self.condition_dim*2, 300)
        self.lin_r1_h1 = nn.Linear(self.h_dim+self.condition_dim*3, 300)
        self.lin_r2_h1 = nn.Linear(self.h_dim+self.condition_dim*4, 300)
        self.lin_T_h1 = nn.Linear(self.h_dim+self.condition_dim*5, 300)
...
class NeuralNetContextRecommender():
    """
    Pytorch vision of 'Using Machine Learning To Predict Suitable Conditions for Organic Reactions'
    ...
    def load_model_from_trained_model(self, trained_model, condition2label, label2condition):
        self.nnModel = trained_model

        self.c1_dict = label2condition['catalyst1']
        self.s1_dict = label2condition['solvent1']
        self.s2_dict = label2condition['solvent2']
        self.r1_dict = label2condition['reagent1']
        self.r2_dict = label2condition['reagent2']

        self.inverse_c1_dict = condition2label['catalyst1']
        self.inverse_s1_dict = condition2label['solvent1']
        self.inverse_s2_dict = condition2label['solvent2']
        self.inverse_r1_dict = condition2label['reagent1']
        self.inverse_r2_dict = condition2label['reagent2']
    ...
    def get_n_conditions(self, rxn, n=10, singleSlvt=True, with_smiles=True, return_scores=False, c1_rank_thres=1, s1_rank_thres=3, s2_rank_thres=1, r1_rank_thres=5, r2_rank_thres=1):
        """
        ...
        try:
            ...
            (top_combos, top_combo_scores) = self.predict_top_combos(
                inputs=inputs,
                c1_rank_thres=c1_rank_thres,
                s1_rank_thres=s1_rank_thres,
                s2_rank_thres=s2_rank_thres,
                r1_rank_thres=r1_rank_thres,
                r2_rank_thres=r2_rank_thres
            )
...
if __name__ == '__main__':
    ...
    cont.load_nn_model(info_path='../data/baseline_data/',
                       weights_path='../data/baseline_data/dict_weights.npy')
