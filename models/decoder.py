...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, args, d_model, heads, d_ff, dropout, self_attn, context_attn):
    ...
    def forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
                previous_input=None, layer_cache=None, prefix_key=None, prefix_value=None):
        """
        Args:
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory_bank, state, tgt_template_mask=None, step=None, cache=None, template_pooling=False, prefix_key=None, prefix_value=None):
        """
        ...
        #     src_pad_mask = torch.cat([torch.zeros(src_pad_mask.shape[0],1).bool().to(src_pad_mask.device),src_pad_mask],dim=1)
        #     src_pad_mask = src_pad_mask.unsqueeze(1).expand(tgt_batch, tgt_len, src_len+1)
        ...
        tgt_reps = torch.einsum("abc,ab->ac", output, (tgt_pad_mask[:, 0, :]==0).float()) 
        ...
        if not template_pooling:
            ...
            tgt_template_reps = torch.einsum("abc,ba->ac", output, tgt_template_mask[1:, :])
...
class DecoderState(object):
    ...
    def beam_update(self, idx, positions, beam_size):
        ...
        for e in self._all:
            sizes = e.size()
            br = sizes[1]  # (batch_size * beam_size)
            if len(sizes) == 3:
