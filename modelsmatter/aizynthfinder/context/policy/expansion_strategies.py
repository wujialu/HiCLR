...
from external.modelsmatter_modelzoo.ssbenchmark.model_zoo import ModelZoo
...
class SmilesBasedExpansionStrategy(ExpansionStrategy):
    ...
    def __init__(self, key: str, config: Configuration, **kwargs: str) -> None:
        ...
        gpu_mode = single_step_model_kwargs.pop('gpu_mode', False)
        module_path = single_step_model_kwargs.pop('module_path')
        ...
        # import pdb
        # pdb.set_trace()
        modelImplementation = ModelZoo(key = key,  module_path = module_path, 
                                       use_gpu = gpu_mode, kwargs=single_step_model_kwargs)
