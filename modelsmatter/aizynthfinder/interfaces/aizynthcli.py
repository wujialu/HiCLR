...
def _get_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--gpu_ids",
        nargs="+",
        help="the list of available gpu ids"
    )
...
def _process_multi_smiles(
    ...
    for smi in smiles:
        ...
        try:
            ...
            continue
        search_time = finder.tree_search(show_progress=True)
        finder.build_routes()
        stats = finder.extract_statistics()
...
def _multiprocess_smiles(args: argparse.Namespace) -> None:
    ...
    log_prefix = os.path.dirname(args.output) + "/aizynthcli"
    start_processes(filenames, log_prefix, create_cmd, gpu_ids=args.gpu_ids)
