...
def start_processes(
    inputs: Sequence[Any], log_prefix: str, cmd_callback: Callable, poll_freq: int = 5, gpu_ids=[0]
) -> None:
    """
    ...
    for index, iinput in enumerate(inputs, 1):
        ...
        env = os.environ.copy()
        env["CUDA_VISIBLE_DEVICES"] = str(gpu_ids[index-1])
        processes.append(
            subprocess.Popen(cmd, stdout=output_fileobjs[-1], stderr=subprocess.STDOUT, env=env)
        )
