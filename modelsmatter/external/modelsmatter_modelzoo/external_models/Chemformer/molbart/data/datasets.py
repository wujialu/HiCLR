...
class ReactionDataset(_AbsDataset):
    ...
    def __getitem__(self, item):
        ...
        output = (
            (reactant, product, self.items[item])
    ...
    def split_idxs(self, val_idxs, test_idxs):
        ...
        val_extra = (
            [self.items[idx] for idx in val_idxs] if self.items is not None else None
        )
        ...
        test_extra = (
            [self.items[idx] for idx in test_idxs] if self.items is not None else None
        )
        ...
        train_extra = (
            [self.items[idx] for idx in train_idxs] if self.items is not None else None
        )
...
class Uspto50(ReactionDataset):
    def __init__(self, data_path, aug_prob, type_token=False, forward=True):
        ...
        if (
            path.is_dir()
        ):  # Added by Paula: parse batched files due to large size #TODO: Clean up if necessary
            reactants = []
            products = []
            type_tokens = []
            for i, f in enumerate(path.iterdir()):
                try:
                    df_ = pd.read_pickle(f.resolve())
                except:
                    print(f"Error reading file {f}")
                    continue
                reactants.extend(df_["reactants_mol"].tolist())
                products.extend(df_["products_mol"].tolist())
                type_tokens.extend(df_["reaction_type"].tolist())
        ...
        super().__init__(
            reactants, products, items=None, transform=None, aug_prob=aug_prob
        )
    ...
    def _prepare_strings(self, react, prod, type_token):
        ...
        if self.forward:
            react_str = (
                f"{str(type_token)}{react_str}" if self.type_token else react_str
            )
        else:
...
class UsptoMixed(ReactionDataset):
    def __init__(self, data_path, aug_prob):
        ...
        super().__init__(
            reactants, products, transform=self._prepare_strings, aug_prob=aug_prob
        )
...
class MoleculeDataset(_AbsDataset):
    ...
    def split_idxs(self, val_idxs, test_idxs):
        ...
        val_lengths = (
            [self.seq_lengths[idx] for idx in val_idxs]
        ...
        test_lengths = (
            [self.seq_lengths[idx] for idx in test_idxs]
            if self.seq_lengths is not None
            else None
        )
        ...
        train_lengths = (
            [self.seq_lengths[idx] for idx in train_idxs]
            if self.seq_lengths is not None
            else None
        )
