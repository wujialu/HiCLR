...
def load_model(args, sampler, vocab_size, total_steps, pad_token_idx):
    ...
    if args.model_path in ["none", "None"]:
        model = load_rand_model(
            args, extra_args, sampler, vocab_size, total_steps, pad_token_idx
        )
    else:
        if args.model_type == "bart":
...
def main(args):
    ...
    if (
        args.store_model
    ):  # Added by Paula: Allow to store the model in specified path # TODO: Clean up if necessary
        print("Saving model")
        args.store_model = os.path.abspath(args.store_model)
        trainer.save_checkpoint(f"{args.store_model}.ckpt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--store_model", type=str, default=False)
