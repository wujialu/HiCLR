...
from rdchiral.template_extractor import extract_from_reaction, get_changed_atoms, mols_from_smiles_list, \
    replace_deuterated, get_tagged_atoms_from_mols
...
def get_reaction_centers_from_template(src_smiles, blank_src_smiles, graph_pack, reaction_centers):
    ...
    for rc in (reaction_centers):
        ...
        if patt is not None and mol_blank.HasSubstructMatch(patt):
            for match in mol_blank.GetSubstructMatches(patt):  #! mol_blank: w/o atom-mapping number for substruct matching
                token_match_indices = []
                for index in match:
                    atom_smarts = mol.GetAtomWithIdx(index).GetSmarts()
                    token_match_indices.append(int(re.match('.*:([0-9]+)\]', atom_smarts).group(1)))  #! return atom-mapping number
...
def get_template_mask(cano_prod_am, cano_reacts_am, rc):
    prod_rc, reacts_rc = rc.split(">>")
    if prod_rc[0] == '(' and rc[-1] == ')':
        prod_rc = prod_rc[1:-1]
    patt = Chem.MolFromSmarts(prod_rc)
    prod_mol = Chem.MolFromSmiles(cano_prod_am)

    if patt is not None and prod_mol.HasSubstructMatch(patt):
        changed_atom_idx = prod_mol.GetSubstructMatch(patt)
        changed_atom_tags = [prod_mol.GetAtomWithIdx(idx).GetProp('molAtomMapNumber') for idx in changed_atom_idx]

    reacts_template_mask = []
    prods_template_mask = []
    for i, token in enumerate(smi_tokenizer(cano_prod_am)):
        if token[0] == '[' and token[-1] == ']' and re.match('.*:([0-9]+)]', token):  #! atom token
            am = re.match('.*:([0-9]+)]', token).group(1)  #! rely on atom-mapping number
            if am in changed_atom_tags:
                prods_template_mask.append(True)
                continue
        prods_template_mask.append(False)

    for i, token in enumerate(smi_tokenizer(cano_reacts_am)):
        if token[0] == '[' and token[-1] == ']' and re.match('.*:([0-9]+)]', token):  
            am = re.match('.*:([0-9]+)]', token).group(1) 
            if am in changed_atom_tags:
                reacts_template_mask.append(True)
                continue
        reacts_template_mask.append(False)
    
    assert sum(reacts_template_mask) == sum(prods_template_mask) == patt.GetNumAtoms()
    assert len(reacts_template_mask) == len(smi_tokenizer(cano_reacts_am))
    assert len(prods_template_mask) == len(smi_tokenizer(cano_prod_am))

    return prods_template_mask, reacts_template_mask
...
def random_rxn(rxn_smi, react_permute_mode = "all"):
    r, reagents, p = rxn_smi.split(">")
    cano_prod_am = randomize_smiles_with_am(p)   
    ...
    random_rxn = cano_reacts + ">>" + cano_prod    
    return random_rxn
...
def canonical_smiles_with_am(smi):
    ...
    origin_mol = Chem.MolFromSmiles(smi)
    atom_ordering = []
    ...
    unmapped_smi = Chem.MolFragmentToSmiles(origin_mol, atomsToUse=atom_ordering, canonical=False)
    ...
    new_tokens = []
    """
    fix a bug: for metal (special-token), add zero-mapping [Au:0] will cause error when removing atom-mapping 
    [Au:0] --> Au (can not be tokenize correctly)
    [C:1] --> C
    """
    for token in smi_tokenizer(Chem.MolToSmiles(mol)):
        if re.match('.*:([0-9]+)]', token):
            pivot = re.match('.*(:[0-9]+])', token).group(1)
            atomIdx = pivot2atomIdx[int(pivot[1:-1])]
            am = atomIdx2am[atomIdx]
            if int(am) == 0:
                token = origin_mol.GetAtomWithIdx(atomIdx).GetSmarts()
            else:
                token = token.replace(pivot, ':{}]'.format(am))
...
def smi_tokenizer(smi):  
    ...
    pattern = "(\[[^\]]+]|Bi|Br?|Ge|Te|Mo|K|Ti|Zr|Y|Na|125I|Al|Ce|Cr|Cl?|Ni?|O|S|Pd?|Fe?|I|b|c|Mn|n|o|s|~|<unk>|<mask>|>>|Li|p|\(|\)|\.|=|#|-|\+|\\\\|\/|:|@|\?|>|\*|\$|\%[0-9]{2}|[0-9])"
    regex = re.compile(pattern)
    tokens = [token for token in regex.findall(smi)]
    try:
        assert smi == ''.join(tokens)
    except AssertionError: #! fix bugs
        print('AssertionError for smi_tokenizer:', smi, ''.join(tokens))
    return tokens
...
def remove_am_without_canonical(smi_am, force_canonical=False):
    ...
    def check_special_token(token): 
        pattern = "(Os|Mg|Zn|Si|Sn|Se|se|Ge|K|Ti|Pd|Mo|Ce|Ta|As|te|Pb|Ru|Ag|W|Pt|Co|Ca|Xe|11CH3|Rh|Tl|V|131I|Re|13c|siH|La|pH|Y|Zr|Bi|125I|Sb|Te|Ni|Fe|Mn|Cr|Al|Na|Li|Cu|nH[0-9]?|NH[1-9]?\+|\+|-|@|PH[1-9]?)"
        regex = re.compile(pattern)
...
def extract_relative_atom_mapping(cano_prod_am, cano_reacts_am):
...
def extract_relative_mapping(cano_prod_am, cano_reacts_am):
    ...
    for i, token in enumerate(cano_reacts_tokens):
        if token[0] == '[' and token[-1] == ']' and re.match('.*:([0-9]+)]', token):
            ...
            if prod_posIdx != -1:
                if (i, prod_posIdx) not in position_mapping_list:
                    #! assertion error: the explicitHs
                    # assert cano_reacts_tokens[i] == cano_prod_tokens[prod_posIdx]
                    position_mapping_list.append((i, prod_posIdx))
                    # print(i, prod_posIdx, cano_reacts_tokens[i], cano_prod_tokens[prod_posIdx])

            # Try to expand with increment
            # #! align both atom tokens and non-atom tokens, e.g. "(", ")", "="
            react_pivot = i + 1
...
def get_nonreactive_mask(cano_prod_am, raw_prod, raw_reacts, radius=0):
    ...
    for i, token in enumerate(smi_tokenizer(cano_prod_am)):
        if token[0] == '[' and token[-1] == ']' and re.match('.*:([0-9]+)]', token):  #! atom token
            am = re.match('.*:([0-9]+)]', token).group(1)  #! atom-mapping number
            if am in changed_atom_tags:
...
class MolTokeniser:
    ...
    def tokenise(self, smiles, pad=True, input_type="src"):
        tokenised = []
        for smi in smiles:
        ...
        if input_type == "src":
            tokens = [[self.src_begin_token] + ts for ts in tokens]
        elif input_type == "tgt":
            tokens = [[self.tgt_begin_token] + ts + [self.tgt_end_token] for ts in tokens]
