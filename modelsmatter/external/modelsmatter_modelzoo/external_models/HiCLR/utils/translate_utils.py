...
def translate_batch(model, batch, device="cuda", eos_idx=3, beam_size=10, max_length=200,
    ...
    for step in range(max_length):
        ...
        with torch.no_grad(): 
            ...
            scores = model.generator(outputs[-1])  # (max_len, beam_size*batch_size, vocab_size_tgt)
        ...
        for j in range(batch_size):
            ...
            if beam_size == eos_index.sum():  # all beam has finished
                ...
                beam_scores = batch_score + prev_score.unsqueeze(1).expand_as(batch_score) # [beam_size,num_words]

                if step == 0:
                    flat_beam_scores = beam_scores[0].view(-1)
                else:
                    flat_beam_scores = beam_scores.view(-1) # beam_size*num_words
                ...
                frozed_tokens = pred_tokens[j][eos_index].clone()
                if frozed_tokens.shape[0] > 0:
                    frozed_tokens[:, step + 1] = eos_idx
                frozed_scores = pred_scores[j][eos_index] 

                # Update the rest of tokens
                origin_tokens = pred_tokens[j][best_scores_id // num_words].clone()  # select the token from prev step
                ...
                #! fix bug in beam_update (state.previous_input = new_input.clone(), else reshuffle the tgt tokens)
