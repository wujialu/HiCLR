...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        #print(atom_labels, bond_labels)
        atom_logits, bond_logits, _ = predict(args, model, bg)
        #print(atom_logits, bond_logits)
        ...
        with torch.no_grad():
            ...
            #del atom_logits, bond_logits, total_loss, loss_a, loss_b, atom_labels, bond_labels, bg, smiles
...
def main(args):
    ...
    #if args['graph_creation_only']:
    #    args['data_dir'] = args['graph_creation_csv_folder']
    #else:
    ...
    if args['graph_creation_only']:
        ...
        print('Graph creation only, exiting...')
        return
    else:
...
if __name__ == '__main__':
    ...
    # change
    parser.add_argument('--graph_creation_only', action='store_true', required=False, default = False)
    parser.add_argument('--model_file_path', type = str, required=True, help = "The file path where the model is saved or loaded from")
    parser.add_argument('--config_path', type = str, required=True, help = "The path where the config file is loaded from")
    parser.add_argument('--template_path', type = str, required=True, help = "The path where the atom/bond templates is loaded from")
    parser.add_argument('--labeled_data_path', type = str, required=True, help = "The path where the labeled data is loaded from")
    parser.add_argument('--graph_file_path', type = str, required=False, help = "The file_path where the graphs are loaded/saved")
