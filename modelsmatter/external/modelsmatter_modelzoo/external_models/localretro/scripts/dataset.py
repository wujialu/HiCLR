...
class USPTODataset(object):
    def __init__(self, args, smiles_to_graph, node_featurizer, edge_featurizer, load=True, log_every=1000):
        df = pd.read_csv(args['labeled_data_path'])
        ...
        self.cache_file_path = args['graph_file_path']
        #if args['graph_creation_only']:
        #    self.cache_file_path = args['graph_creation_folder'] + '/%s_graphs.bin' % args['dataset']
        #else:
...
class USPTOTestDataset(object):
    def __init__(self, args, smiles_to_graph, node_featurizer, edge_featurizer, load=True, log_every=1000):
        ...
        self.cache_file_path = args['graph_file_path'] + '_test.bin'
        self._pre_process(smiles_to_graph, node_featurizer, edge_featurizer, load, log_every)
