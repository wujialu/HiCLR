...
def get_configure(args):
    ...
    AtomTemplate_n = len(pd.read_csv('%s/atom_templates.csv' % args['template_path']))
    BondTemplate_n = len(pd.read_csv('%s/bond_templates.csv' % args['template_path']))
...
def load_dataloader(args):
    if args['mode'] == 'train':
        print("Training Mode Data Loading")
...
def load_model(args):
    ...
    if args['mode'] == 'train':
        ...
        if os.path.exists(args['model_file_path']):
            stopper = EarlyStopping(mode = 'lower', patience=args['patience'], filename=args['model_file_path'])
        ...
        model.load_state_dict(torch.load(args['model_file_path'], map_location=torch.device('cpu'))['model_state_dict'])
        return model
