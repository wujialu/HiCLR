# Changes from ModelsMatter
- We change the logic such that for large datasets the templates are converted to fingerprints then cached and loaded as needed for training and testing, due to RAM limitations
- We add the option of a cutoff such that templates that only occur < N times are discarded
