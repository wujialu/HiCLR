...
def get_uspto_480k():
    ...
    download_file(
        "https://github.com/connorcoley/rexgen_direct/raw/master/rexgen_direct/data/test.txt.tar.gz", "test.txt.tar.gz"
    )
    subprocess.run(["tar", "zxf", "test.txt.tar.gz"])

    with open("train.txt") as f:
        train = [{"reaction_smiles": line.strip(), "split": "train"} for line in f.readlines()]
    with open("valid.txt") as f:
        valid = [{"reaction_smiles": line.strip(), "split": "valid"} for line in f.readlines()]
    with open("test.txt") as f:
        test = [{"reaction_smiles": line.strip(), "split": "test"} for line in f.readlines()]

    df = pd.concat([pd.DataFrame(train), pd.DataFrame(valid), pd.DataFrame(test)]).reset_index()
...
def get_uspto_golden():
    ...
    subprocess.run(
        [
            "wget",
            "https://github.com/Laboratoire-de-Chemoinformatique/Reaction_Data_Cleaning/raw/master/data/golden_dataset.zip",
        ]
    )
...
def load_USPTO_fortu(path="data/processed", which="uspto_sm_", is_appl_matrix=False):
    ...
    for split in ["train", "valid", "test"]:
        ...
        print(
            ...
            y[split].shape[0],
            "samples (",
            y[split].max() if not is_appl_matrix else y[split].shape[1],
            "max label)",
        )
    return X, y
...
def load_dataset_from_csv(
    ...
    **kwargs,
    ...
    if (
        ...
        mre = ":\d+(?=])"
        df["unmapped_template"] = df["reaction_smarts"].apply(lambda x: re.sub(mre, "", x))
    ...
    if ssretroeval:
        ...
        for ii in only_in_test:
            template_list[
                ii
            ] = "CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC.CCCCCCCCCCCCCCCCCCCCCCCCCCC.CCCCCCCCCCCCCCCCCCCCCC>>CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC.CCCCCCCCCCCCCCCCCCCCC"  # obfuscate them
        if ret_df:
