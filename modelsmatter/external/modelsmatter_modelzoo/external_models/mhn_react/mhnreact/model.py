...
from .plotutils import plot_loss, plot_nte, plot_topk
...
class ChemRXNDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        cached_fp=False,
        split="train",
    ):  # TODO: Clean up options, they can be reduced
        self.is_smiles = is_smiles
        self.cached_fp = cached_fp
        if cached_fp:
            self.Xs = Xs
            self.target = target
            self.cached_fp = cached_fp
            ...
            is_smiles
        ):  # TODO: check the logic order here, should it be cached then is_smiles?
            self.Xs = Xs
            self.target = target
    ...
    def __getitem__(self, k):
        if self.cached_fp:
            if self.Xs == self.target:
                with h5py.File(self.Xs, "r") as f:
                    mol_fp = f[f"molecules_{self.split}"][k, :]
                    target = mol_fp.copy()
            else:
                with h5py.File(self.Xs, "r") as f:
                    mol_fp = f[f"molecules_{self.split}"][k, :]
                with h5py.File(self.target, "r") as f:
                    target = f[f"molecules_{self.split}"][k, :]
        else:
            mol_fp = self.Xs[k]
            if self.is_smiles:
                mol_fp = convert_smiles_to_fp(
                    mol_fp, fp_size=self.fp_size, which=self.fingerprint_type
                ).astype(np.float32)

            target = None if self.target is None else self.target[k]
            if (self.is_smiles == True) and self.target:
                target = convert_smiles_to_fp(
                    target, fp_size=self.fp_size, which=self.fingerprint_type
                ).astype(np.float32)
    ...
    def __len__(self):
        if not self.cached_fp:
            return len(self.Xs)
        else:
            with h5py.File(self.Xs, "r") as f:
                len_X = f[f"molecules_{self.split}"].len()  # Added by Paula
            return len_X
...
class ModelConfig(object):
    def __init__(self, **kwargs):
        ...
        self.pooling_operation_state_embedding = kwargs.pop(
            "pooling_operation_state_embedding", "mean"
        )
        ...
        self.verbose = kwargs.pop(
            "verbose", False
        )  # debugging or printing additional warnings / information set tot True
        ...
        self.hopf_association_activation = kwargs.pop(
            "hopf_association_activation", None
        )
        self.hopf_beta = kwargs.pop(
            "hopf_beta", 0.125
        )  #  1/(self.hopf_asso_dim**(1/2) sqrt(d_k)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.norm_in = (
            (lambda k: k)
            if not norm_in
            else torch.nn.LayerNorm(input_size, elementwise_affine=False)
        )
        self.norm_out = (
            (lambda k: k)
            if not norm_out
            else torch.nn.LayerNorm(output_size, elementwise_affine=False)
        )
    ...
    def setup_ff(self, input_size: int, output_size: int, num_layers=1):
        ...
        for n in range(0, num_layers):
            ...
            torch.nn.init.kaiming_normal_(
                w.weight, mode="fan_in", nonlinearity="linear"
            )  # eqiv to LeCun init
...
class FPMolEncoder(Encoder):
    ...
    def __init__(self, config):
        super().__init__(
            input_size=config.hopf_input_size * config.hopf_num_heads,
    ...
    def convert_smiles_to_tensor(self, list_of_smiles):
        fps = convert_smiles_to_fp(
            list_of_smiles,
            fp_size=self.config.fp_size,
            which=self.config.fingerprint_type,
            radius=self.config.fp_radius,
        )
        fps_tensor = (
            torch.from_numpy(fps.astype(np.float))
...
class MHN(nn.Module):
    ...
    def __init__(self, config=None, layer2weight=0.05, use_template_encoder=True):
        ...
        if hasattr(self.config, "hopf_n_layers"):
            ...
            if di["hopf_n_layers"] > 0:
                ...
                if di["hopf_n_layers"] != 0:
                    self.W_v = nn.Linear(
                        self.config.hopf_asso_dim, self.config.hopf_input_size
                    )
                    torch.nn.init.kaiming_normal_(
                        self.W_v.weight, mode="fan_in", nonlinearity="linear"
                    )  # eqiv to LeCun init
        ...
        self.pretrain_lossfunction = nn.BCEWithLogitsLoss(
            reduction="none"
        )  # , weight=class_weights)

        self.lr = self.config.lr

        if self.config.hopf_association_activation is None or (
            self.config.hopf_association_activation.lower() == "none"
        ):
            self.af = lambda k: k
        else:
    ...
    def set_templates(
        ...
        if len(template_list) >= 100000:
            ...
            for b in range(math.ceil(len(template_list) // bs) + 1):
                ...
                templ_emb = self.update_template_embedding(
                    which=which,
                    fp_size=fp_size,
                    radius=radius,
                    learnable=learnable,
                    njobs=njobs,
                    only_templates_in_batch=only_templates_in_batch,
                )
                final_temp_emb[
                    bs * b : min(bs * (b + 1), len(template_list))
                ] = torch.from_numpy(templ_emb)
            self.templates = final_temp_emb
        else:
            final_temp_emb = self.update_template_embedding(
                which=which,
                fp_size=fp_size,
                radius=radius,
                learnable=learnable,
    ...
    def update_template_embedding(
        ...
        split_template_list = [
            str(t).split(">")[0].split(".") for t in self.template_list
        ]
        templates_np = convert_smiles_to_fp(
            split_template_list,
            is_smarts=True,
            fp_size=fp_size,
            radius=radius,
        ...
        split_template_list = [
            str(t).split(">")[-1].split(".") for t in self.template_list
        ]
        reactants_np = convert_smiles_to_fp(
            split_template_list,
            is_smarts=True,
            fp_size=fp_size,
            radius=radius,
        ...
        if learnable:
            self.templates = torch.nn.Parameter(
                torch.from_numpy(template_representation).float(), requires_grad=True
            ).to(self.config.device)
            ...
            if only_templates_in_batch:
                ...
                self.templates = (
                    torch.from_numpy(template_representation)
                    .float()
                    .to(self.config.device)
                )
    ...
    def masked_loss_fun(self, loss_fun, h_out, ys_batch):
        ...
        loss = (
            loss_fun(h_out, ys_batch * mask) * mask.float()
        ).sum() / mask_sum  # only mean from non -1
        return loss

    def compute_losses(self, out, ys_batch, head_loss_weight=None):
        if len(ys_batch.shape) == 2:
            if (
                ys_batch.shape[1] == self.config.num_templates
            ):  # it is in pretraining_mode
                loss = self.pretrain_lossfunction(out, ys_batch.float()).mean()
            else:
                # legacy from policyNN
                loss = self.lossfunction(
                    out, ys_batch[:, 2]
                ).mean()  # WARNING: HEAD4 Reaction Template is ys[:,2]
        else:
    ...
    def forward(
        ...
        if (templates is None) and (self.X is None) and (self.templates is None):
            raise Exception(
                "Either pass in templates, or init templates by running clf.set_templates"
            )

        n_temp = len(templates) if templates is not None else len(self.templates)

        if (
            self.training or (templates is None) or (self.X is None)
        ):  # Added by Paula:  Changed this to self.X is None, not sure if correct
        ...
        Xi = Xi.view(
            bs, self.config.hopf_num_heads, self.config.hopf_asso_dim
        )  # [bs, H, A]
        X = X.view(
            1, n_temp, self.config.hopf_asso_dim, self.config.hopf_num_heads
        )  # [1, T, A, H]
        ...
        if self.config.hopf_num_heads <= 1:
            ...
            XXi = self.pooling_operation_head(
                XXi, dim=2
            )  # default is max pooling over H [bs, T]
            if (self.config.pooling_operation_head == "max") or (
                self.config.pooling_operation_head == "min"
            ):
                XXi = XXi[0]  # max and min also return the indices =S

        out = self.beta * XXi  # [bs, T, H] # softmax over dim=1 #pooling_operation_head

        self.xinew = self.softmax(out) @ X.view(
            n_temp, self.config.hopf_asso_dim
        )  # [bs,T]@[T,emb] -> [bs,emb]
        ...
        if self.W_v:
            ...
            hopfout = self.W_v(
                self.xinew
            )  # [bs,emb]@[emb,hopf_inp]  --> [bs, hopf_inp]
    ...
    def setup_batch(
        ...
        ys_batch,
        large_dataset,
        template_cache_id=None,
        cachedir=None,
        ttest=False,
    ):
        if large_dataset.cached_fp is not None:
            template_representation = np.empty(
                shape=(len(ys_batch), large_dataset.fp_size)
            )
            templates = ys_batch.tolist()
            t = list(np.unique(templates))
            t_order = [t.index(n) for n in templates]
            with h5py.File(large_dataset.cached_fp, "r") as f:
                template_representation = f["templates"][t, :]
            # with h5py.File(large_dataset.cached_fp, "r") as f:
            #     template_representation = f["templates"][:, :]
            # template_representation[:] = template_representation_[t_order]
            ys_batch = torch.tensor(
                t_order, dtype=torch.long, device=self.config.device
            )
            templates = (
                torch.from_numpy(template_representation).float().to(self.config.device)
            )
            ...
            if cachedir is not None:
                if not hasattr(self, "template_cache"):
                    print("set up template cache")
                    self.template_cache = collections.defaultdict(None)
                if template_cache_id in self.template_cache.keys():
                    print("used cached templates")
                    self.templates = torch.load(self.template_cache[template_cache_id])
                    self.set_templates_recursively()
                    templates = self.templates
                    return templates, ys_batch
            print("START SETUP BATCH", len(ys_batch))
            templates, ys_batch = large_dataset.get_template_features(
                ys_batch, ttest=ttest
            )
            print("AFTER GET TEMPLATE FEATURES", len(templates))
            ys_batch = ys_batch.to(self.config.device)
            ...
            templates = self.set_templates(
                ...
                learnable=large_dataset.learnable,
                njobs=large_dataset.njobs,
                only_templates_in_batch=large_dataset.only_templates_in_batch,
                large_dataset=True,
            )
            print("AFTER SET", len(templates))
            templates = torch.from_numpy(templates).float().to(self.config.device)
            if cachedir is not None:
                tmp_cache_fp = os.path.join(cachedir, f"tmp_{template_cache_id}.pt")
                torch.save(templates, tmp_cache_fp)
                self.template_cache[template_cache_id] = tmp_cache_fp
            print(len(templates))
        return templates, ys_batch
    ...
    def train_from_np(
        ...
        if optimizer is None:
            try:
                self.optimizer = getattr(torch.optim, self.config.optimizer)(
                    self.parameters(), lr=self.lr if lr is None else lr
                )
            except AttributeError as err:
                log.error(f"Can't find optimizer {config.optimizer} in torch.optim")
                raise err
            optimizer = self.optimizer
        dataset = ChemRXNDataset(
            Xs
            if (large_dataset is None) or (large_dataset.cached_fp is None)
            else large_dataset.cached_fp,
            targets
            if (large_dataset is None) or (large_dataset.cached_fp is None)
            else large_dataset.cached_fp,
            ys,
            ...
            cached_fp=bool(large_dataset.cached_fp)
        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            for step, s in tqdm(enumerate(dataloader), mininterval=2):
                ...
                if large_dataset is None:
                    ...
                    templates, ys_batch = self.setup_batch(
                        ys_batch=ys_batch,
                        large_dataset=large_dataset,
                        template_cache_id=step,
                        cachedir=cachedir,
                    )
                    out = self.forward(
                        Xs_batch,
                ...
                rs = min(100, len(Xs) // bs)  # reporting/logging steps # DELETE THIS
                print(
                    "[%d, %5d] loss: %.3f" % (epoch + 1, step + 1, running_loss / rs)
                )  # DELETE THIS
                ...
                if step % rs == (rs - 1):  # print every 2000 mini-batches
                    ...
                    [
                        self.hist[k].append(running_loss_dict[k] / rs)
    ...
    def evaluate(
        ...
        y_preds = np.zeros(
            (
                ys.shape[0],
                len(large_dataset.template_list)
                if large_dataset is not None
                else self.config.num_templates,
            ),
            dtype=np.float16,
        )
        ...
        with torch.no_grad():
            ...
            dataset = ChemRXNDataset(
                ...
                fingerprint_type=self.config.fingerprint_type,
                cached_fp=bool(large_dataset.cached_fp)
                if (large_dataset is not None) and (split != "ttest")
                else False,
                split=split,
            )
            dataloader = torch.utils.data.DataLoader(
            ...
            for step, batch in enumerate(dataloader):  #
                ...
                if hasattr(self, "templates_np"):
                    ...
                    for ii in range(10):
                        ...
                        templates = (
                            torch.from_numpy(
                                self.templates_np[
                                    (i_tlen * ii) : min(i_tlen * (ii + 1), tlen)
                                ]
                            )
                            .float()
                            .to(self.config.device)
                    ...
                    if large_dataset is None:
                        ...
                        outputs = []
                        tlen = len(large_dataset.template_list)
                        i_tlen = tlen // bs
                        # template_list = list(large_dataset.template_list.values())

                        for ii in range(i_tlen + 1):
                            templates = list(range((bs * ii), min(bs * (ii + 1), tlen)))
                            print(max(templates))
                            templates, _ = self.setup_batch(
                                ys_batch=torch.tensor(templates),
                                large_dataset=large_dataset,
                                template_cache_id=ii,
                                cachedir=cachedir,
                                ttest=split == "ttest",
                            )
                            outputs.append(
                ...
                if len(ys.shape) > 1:
                    outputs = (
                        ...
                        if not (ys.shape[1] == self.config.num_templates)
                        else torch.sigmoid(outputs)
                    )
                else:
                    outputs = self.softmax(outputs)
                print("OUTPUTS", outputs.shape, ys_batch.shape)
                if large_dataset is not None:
                    print("YPREDS", y_preds.shape, len(large_dataset.template_list))
                outputs_np = [
                    None if o is None else o.to("cpu").numpy().astype(np.float16)
                    for o in outputs
                ]
                ...
                if not only_loss:
                    ...
                    topkacc, mrocc = top_k_accuracy(
                        ys_batch, outputs, k=ks, ret_arocc=True, ret_mrocc=False
                    )
                    # mrocc -- median rank of correct choice
                    for k, tkacc in zip(ks, topkacc):
                        # iterative average update
                        new_hist[f"t{k}_acc_{split}"] += (
                            tkacc - new_hist[f"t{k}_acc_{split}"]
                        ) / (step + 1)
    ...
    def save_hist(self, prefix="", postfix="", model_path=".data/model/"):
        HIST_PATH = os.path.join(model_path, "hist/")
        if not os.path.exists(HIST_PATH):
            os.mkdir(HIST_PATH)
        fn_hist = os.path.join(HIST_PATH, prefix + postfix + ".csv")
        with open(fn_hist, "w") as fh:
            print(dict(self.hist), file=fh)
        return fn_hist

    def save_model(
        self, prefix="", postfix="", name_as_conf=False, model_path="data/model/"
    ):
        print("saving model")  # DELETE THIS
        if name_as_conf:
            confi_str = (
                str(self.config.__dict__.values())
                .replace("'", "")
                .replace(": ", "_")
                .replace(", ", ";")
            )
...
class StaticQK(MHN):
    ...
    def update_template_embedding(
        self, which="rdk", fp_size=2048, radius=4, learnable=False
    ):
        bs = self.config.batch_size
...
class Retrosim(StaticQK):
    ...
    def fit_with_train(self, X_fp_train, y_train):
        ...
        self.sample2acttemplate = torch.nn.functional.one_hot(
            torch.from_numpy(y_train), self.config.num_templates
        ).float()
        tmpnorm = self.sample2acttemplate.sum(0)
        tmpnorm[tmpnorm == 0] = 1
        self.sample2acttemplate = (self.sample2acttemplate / tmpnorm).to(
            self.config.device
        )  # results in an average after dot product
