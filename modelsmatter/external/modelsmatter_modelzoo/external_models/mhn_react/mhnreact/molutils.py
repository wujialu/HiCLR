...
def getFingerprint(
    smiles, fp_size=4096, radius=2, is_smarts=False, which="morgan", sanitize=True
):
    """maccs+morganc+topologicaltorsion+erg+atompair+pattern+rdkc"""
    if isinstance(smiles, list):
        return np.array(
            [getFingerprint(smi, fp_size, radius, is_smarts, which) for smi in smiles]
        ).max(
            0
        )  # max pooling if it's list of lists
    ...
    mol.UpdatePropertyCache(
        strict=False
    )  # Correcting valence info # important operation
    ...
    def mol2np(mol, which, fp_size):
        ...
        if which == "morgan":
            ...
            fp = AllChem.UnfoldedRDKFingerprintCountBased(
                mol, maxPath=6
            ).GetNonzeroElements()
            is_dict = True
        elif which == "morganc":
    ...
    if ("+" in which) or (cc_symb in which):
        ...
        for fp_type in which.split(split_sym):
            if concat:
                ...
                np_fp[
                    (fp_size - fp_length_remain) : (
                        fp_size - fp_length_remain + len(fpp)
                    )
                ] += fpp
...
def convert_smiles_to_fp(
    ...
    if njobs > 0:
        ...
        fps = [
            getFingerprint(
                smi, fp_size=fp_size, radius=radius, is_smarts=is_smarts, which=which
            )
...
def convert_smartes_to_fp(list_of_smarts, fp_size=2048):
    ...
    if isinstance(list_of_smarts, list):
        if isinstance(list_of_smarts[0], list):
            ...
            fps = [
                [getTemplateFingerprint(smarts, fp_size=fp_size) for smarts in sample]
                + [np.zeros(fp_size, dtype=np.bool)]
                * (pad - len(sample))  # zero padding
                for sample in list_of_smarts
            ]
        else:
            fps = [
                [
                    getTemplateFingerprint(smarts, fp_size=fp_size)
                    for smarts in list_of_smarts
                ]
            ]
...
def mol2morganfp(mol, radius=2, fp_size=2048):
    try:
        Chem.SanitizeMol(
            mol
        )  # due to error --> see https://sourceforge.net/p/rdkit/mailman/message/34828604/
    except:
        pass
        # print(mol)
        # return np.zeros(fp_size).astype(np.bool)
...
class FP_featurizer:
            ...
            try:
                ...
                if is_smarts:
                    ...
                    mol = Chem.MolFromSmiles(
                        smiles
                    )  # TODO small hack only applicable here!!!
                fp_dict = {}
    ...
    def fit(self, x_train, is_smarts=False):
        ...
        self.idx_col = (
            (-np.array(var_col))
            .flatten()
            .argpartition(min(self.max_features, Xraw.shape[1] - 1))[
                : min(self.max_features, Xraw.shape[1])
            ]
        )
...
def calc_template_fingerprint_group_mapping(template_list, fp_size, save_path=""):
    ...
    templ_df["templ_emb"] = templ_df["smarts"].swifter.apply(
        lambda smarts: str(list(getTemplateFingerprintOnBits(smarts, fp_size)))
    )
    ...
    new_templ_df = templ_df.join(
        grouped_templ, on="templ_emb", how="right", lsuffix="_l", rsuffix="_r"
    ).sort_values("idx_orig_l")
    ...
    if save_path != "":
        ...
        np.save(
            f"{save_path}{suffix_takethose}",
            take_those_indices_from_orig,
            allow_pickle=False,
        )
...
class ECFC_featurizer:
    def __init__(
        self, radius=6, min_fragm_occur=50, useChirality=True, useFeatures=False
    ):
        self.v = DictVectorizer(sparse=True, dtype=np.uint16)
...
def ecfp2dict(mol, radius=3):
    ...
    v = MHFPEncoder.secfp_from_mol(
        mol, length=4068, radius=radius, rings=True, kekulize=True, min_radius=1
    )
...
def morgan2dict(
    mol, radius=2, useChirality=True, useBondTypes=True, useFeatures=True, useConts=True
):
    mdic = AllChem.GetMorganFingerprint(
        mol,
        radius=radius,
    ...
    return {
        f"m{radius}{useChirality}{useBondTypes}{useFeatures}" + str(kk): mdic[kk]
        for kk in mdic
    }
...
fingerprintTypes = {
    "MACCS": lambda k: {
        "MCCS" + str(ob): 1 for ob in AllChem.GetMACCSKeysFingerprint(k).GetOnBits()
    },
...
def smarts2appl(
    ...
    inps = [
        (ps, template_product_smarts, template_ints, fpsize, v, use_tqdm, ioff, 0)
        for ps, ioff in zip(product_splits, ioffsets)
    ]
...
def getTemplateFingerprint(smarts, fp_size=4096):
    ...
    return np.array(
        list(
            AllChem.CreateStructuralFingerprintForReaction(
                rxn,
            )
        ),
        dtype=np.bool,
...
class LargeDataset:
    def __init__(
        ...
        self.template_fp_type = template_fp_type
        ...
        if cachedir is not None:
            ...
            self.cached_fp = os.path.join(cachedir, f"data.hdf5")
            # delete old cache
            if os.path.exists(self.cached_fp):
                os.remove(self.cached_fp)
        else:
            self.cached_fp = None

    def get_template_features(self, ys_batch, ttest=False):
        template_dict = {}
        templates = []
        ys_batch_update = []
        i = 0
        for y in ys_batch.tolist():
            if not ttest:
                if self.template_list[y] not in templates:
                    template_dict[self.template_list[y]] = i
                    templates.append(self.template_list[y])
                    ys_batch_update.append(i)
                    i += 1
                else:
                    templates.append(self.template_list[y])
                    ys_batch_update.append(y)
            else:
                print(template_dict)
                ys_batch_update.append(template_dict[self.template_list[y]])
        ys_batch = torch.tensor(ys_batch_update)
        templates = list(templates)

        return templates, ys_batch

    def get_molecule_features(self, X):
        Xs_batch = convert_smiles_to_fp(
            X,
