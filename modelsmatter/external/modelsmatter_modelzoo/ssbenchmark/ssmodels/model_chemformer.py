...
from external_models.Chemformer.molbart.data.datasets import ReactionDataset
from external_models.Chemformer.molbart.predict import predict
import external_models.Chemformer.molbart.util as util
from external_models.Chemformer.molbart.decoder import DecodeSampler
from external_models.Chemformer.molbart.data.datamodules import FineTuneReactionDataModule
...
class model_chemformer(SSMethod):
    ...
    def build_datamodule(self, args, dataset, tokeniser, max_seq_len):
        ...
        dm = FineTuneReactionDataModule(
            ...
            val_idxs=[],
            test_idxs=test_idxs,
            forward_pred=False if args.task == "backward_prediction" else True
        )
        return dm
    ...
    def _model_call(self, X):
        ...
        # relative probabilities among the top-N candidates 
        output = F.softmax(
            torch.Tensor(log_lhs), dim=1
        )  # Added by Paula: AZF requires probabilities not log_lhs (#?? log-likelihoods need length correction)
