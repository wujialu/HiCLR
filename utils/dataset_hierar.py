...
class RetroDataset(Dataset):
    def __init__(self, mode, data_folder='./data',
        ...
        if shared_vocab:
            ...
            vocab_file += 'vocab.pk'
        vocab_path = os.path.join(os.path.dirname(data_folder), vocab_file)
...
class SimpleRetroDataset(RetroDataset):
    def __init__(self, mode, data_folder='./data',
                 known_class=False, shared_vocab=False, augment=False, sample=False, data_file=None,
                 sample_per_class=8, random_state=0):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train' and p > 0.5 and self.augment:
            ...
            rt = processed['reaction_class']
            try:
                src, tgt, context_align = self.parse_smi(prod, react, rt, randomize=True)
            except:
        ...
        if reagents is not "":
            reagents = list(AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reagents),2,512).ToBitString())
            return src, tgt, reaction_class, eval(reaction_id), torch.tensor(np.array(reagents, dtype=int)), context_align
        else:
...
class PCLRetroDataset(RetroDataset):
    ...
    def parse_smi_wrapper(self, task):
        prod, reacts, react_class, retro_template = task
        if not prod or not reacts:
            return None
        return self.parse_smi(prod, reacts, react_class, retro_template, build_vocab=False, randomize=False)

    def parse_smi(self, prod, reacts, react_class, retro_template=None, build_vocab=False, randomize=False, react_permute_mode="order"): 
        ''' Process raw atom-mapped product and reactants into model-ready inputs
        :param prod: atom-mapped product
        :param reacts: atom-mapped reactants
        :param react_class: reaction class
